export const THINKING_SUMMARY_MAX_CHARS = 500;
...
export function getMessageInfo(value) {
    if (!isRecord(value))
        return undefined;
    ...
    return {
        ...
        tools: isRecord(info.tools)
            ? Object.entries(info.tools).reduce((acc, [key, value]) => {
                if (value === true ||
                    value === false ||
                    value === "allow" ||
                    value === "deny" ||
...
export function getMessageParts(value) {
    if (!isRecord(value))
        return [];
...
export function isUnstableTask(task) {
...
export function buildReminder(task, summary, idleMs) {
