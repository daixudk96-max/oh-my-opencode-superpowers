...
export async function findToolResultsBySizeFromSDK(client, sessionID) {
    try {
...
export async function truncateToolResultAsync(client, sessionID, messageID, partId, part) {
...
export async function countTruncatedResultsFromSDK(client, sessionID) {
    try {
        const response = await client.session.messages({ path: { id: sessionID } });
        const messages = normalizeSDKResponse(response, [], { preferResponseOnMissingData: true });
        let count = 0;
...
export async function getTotalToolOutputSizeFromSDK(client, sessionID) {
