...
export function findToolResultsBySize(sessionID) {
...
export function findLargestToolResult(sessionID) {
    const results = findToolResultsBySize(sessionID);
    return results.length > 0 ? results[0] : null;
}
export function truncateToolResult(partPath) {
    if (isSqliteBackend()) {
...
export function getTotalToolOutputSize(sessionID) {
    const results = findToolResultsBySize(sessionID);
    return results.reduce((sum, result) => sum + result.outputSize, 0);
}
export function countTruncatedResults(sessionID) {
    const messageIds = getMessageIds(sessionID);
    let count = 0;
    for (const messageID of messageIds) {
        const partDir = join(PART_STORAGE_DIR, messageID);
