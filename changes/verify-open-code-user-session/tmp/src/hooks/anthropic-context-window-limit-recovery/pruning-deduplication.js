...
export function createToolSignature(toolName, input) {
...
async function readMessagesFromSDK(client, sessionID) {
    try {
...
export async function executeDeduplication(sessionID, state, config, protectedTools, client) {
    if (!config.enabled)
        return 0;
    ...
    for (const msg of messages) {
        ...
        for (const part of msg.parts) {
            ...
            if (config.protectedTools?.includes(part.tool))
                continue;
            if (state.toolIdsToPrune.has(part.callID))
                continue;
