...
export function extractMessageIndex(error) {
    try {
...
export function extractUnavailableToolName(error) {
    try {
        const message = getErrorMessage(error);
...
export function detectErrorType(error) {
    try {
        const message = getErrorMessage(error);
        if (message.includes("assistant message prefill") ||
        ...
        if (message.includes("dummy_tool") ||
