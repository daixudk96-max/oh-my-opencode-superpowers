...
export function createToolExecuteBeforeHandler(ctx, config) {
    return async (input, output) => {
        if (input.tool.trim() === "todowrite" && typeof output.args.todos === "string") {
            ...
            try {
                ...
                throw new Error(`[todowrite ERROR] Failed to parse todos string as JSON. ` +
                    `Received: ${output.args.todos.length > 100
                        ? output.args.todos.slice(0, 100) + "..."
                        : output.args.todos} ` +
                    `Expected: Valid JSON array. Pass todos as an array, not a string.`);
            }
            if (!Array.isArray(parsed)) {
                throw new Error(`[todowrite ERROR] Parsed JSON is not an array. ` +
