 ...
 */
function isSafeContext(content, matchStart, matchEnd) {
...
function hasBashRedirectionOperator(command) {
    return /(?:^|\s)(?:>>|2>|>)\s*\S+/.test(command);
}
/**
 * Scan content for secrets
 */
export function scanContent(content, filePath, config = DEFAULT_SECRET_SCANNER_CONFIG) {
    // Check whitelist
 ...
 */
export function createSecretScannerHook(ctx, options = {}) {
    const config = {
    ...
    return {
        ...
        "tool.execute.before": async (input, output) => {
            ...
            if (toolLower === "bash" ||
                ...
                toolLower === "command" ||
                toolLower === "execute_bash") {
                const command = (output.args.command ?? output.args.cmd ?? (Array.isArray(output.args.args) ? output.args.args[0] : undefined));
                if (!command) {
                    return;
                }
