...
export function getServerBaseUrl(client) {
...
export async function patchPart(client, sessionID, messageID, partID, body) {
...
export async function deletePart(client, sessionID, messageID, partID) {
    const baseUrl = getServerBaseUrl(client);
    if (!baseUrl) {
        log("[opencode-http-api] Could not extract baseUrl from client");
        return false;
