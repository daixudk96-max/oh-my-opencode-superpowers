...
export function readConfiguredProviders() {
    ...
    if (detected.format === "none")
        return null;
    const config = readJsoncFile(detected.path);
...
export function fuzzyMatchModel(target, available, providers) {
    ...
    if (exactModelIdMatches.length > 0) {
        const result = exactModelIdMatches.reduce((shortest, current) => current.length < shortest.length ? current : shortest);
 ...
 */
export function isModelAvailable(targetModel, availableModels) {
    return fuzzyMatchModel(targetModel, availableModels) !== null;
}
export async function getConnectedProviders(client) {
...
export async function fetchAvailableModels(client, options) {
...
export function isModelCacheAvailable() {
