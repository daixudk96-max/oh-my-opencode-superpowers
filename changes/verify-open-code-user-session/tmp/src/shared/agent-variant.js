...
export function resolveAgentVariant(config, agentName) {
    if (!agentName) {
        return undefined;
    }
...
export function resolveVariantForModel(config, agentName, currentModel) {
    const agentOverrides = config.agents;
    const agentOverride = agentOverrides
        ? agentOverrides[agentName]
            ?? Object.entries(agentOverrides).find(([key]) => key.toLowerCase() === agentName.toLowerCase())?.[1]
...
function findVariantInChain(fallbackChain, currentModel) {
...
export function applyAgentVariant(config, agentName, message) {
