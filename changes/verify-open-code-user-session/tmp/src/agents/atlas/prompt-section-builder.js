...
export const getCategoryDescription = (name, userCategories) => userCategories?.[name]?.description ?? CATEGORY_DESCRIPTIONS[name] ?? "General tasks";
export function buildAgentSelectionSection(agents) {
    if (agents.length === 0) {
...
export function buildCategorySection(userCategories) {
...
export function buildDecisionMatrix(agents, userCategories) {
    const allCategories = mergeCategories(userCategories);
