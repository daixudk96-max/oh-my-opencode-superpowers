...
import { fetchAvailableModels, readConnectedProvidersCache, readProviderModelsCache, } from "../shared";
...
export async function createBuiltinAgents(disabledAgents = [], agentOverrides = {}, directory, systemDefaultModel, categories, gitMasterConfig, discoveredSkills = [], customAgentSummaries, browserProvider, uiSelectedModel, disabledSkills, useTaskSystem = false, disableOmoEnv = false) {
    ...
    const mergedAgentSources = { ...agentSources };
    ...
    for (const manifest of downstreamAgentManifests) {
        ...
        if (!normalizedName)
            continue;
        if (normalizedName in mergedAgentSources)
            continue;
        mergedAgentSources[normalizedName] = manifest.factory;
        if (manifest.metadata) {
            mergedAgentMetadata[normalizedName] = manifest.metadata;
        }
    }
