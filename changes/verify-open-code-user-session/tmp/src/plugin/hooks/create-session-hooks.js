...
import { detectExternalNotificationPlugin, getNotificationConflictWarning, log, normalizeSDKResponse, } from "../../shared";
...
export function createSessionHooks(args) {
    ...
    const preemptiveCompaction = isHookEnabled("preemptive-compaction") &&
        ...
        ? safeHook("preemptive-compaction", () => createPreemptiveCompactionHook(ctx, pluginConfig, modelCacheState))
        : null;
    const sessionRecovery = isHookEnabled("session-recovery")
        ? safeHook("session-recovery", () => createSessionRecoveryHook(ctx, { experimental: pluginConfig.experimental }))
        : null;
    ...
    const updateFallbackTitle = async (input) => {
        ...
        if (!existing.baseTitle) {
            ...
            const sessionInfo = sessionResp
                ? normalizeSDKResponse(sessionResp, null, { preferResponseOnMissingData: true })
                : null;
    ...
    const modelFallback = isModelFallbackConfigEnabled && isHookEnabled("model-fallback")
        ? safeHook("model-fallback", () => createModelFallbackHook({
    ...
    const anthropicContextWindowLimitRecovery = isHookEnabled("anthropic-context-window-limit-recovery")
        ? safeHook("anthropic-context-window-limit-recovery", () => createAnthropicContextWindowLimitRecoveryHook(ctx, { experimental: pluginConfig.experimental, pluginConfig }))
        : null;
    const autoUpdateChecker = isHookEnabled("auto-update-checker")
        ? safeHook("auto-update-checker", () => createAutoUpdateCheckerHook(ctx, {
    ...
    const anthropicEffort = isHookEnabled("anthropic-effort")
        ...
        : null;
    const runtimeFallbackConfig = typeof pluginConfig.runtime_fallback === "boolean"
        ? { enabled: pluginConfig.runtime_fallback }
        : pluginConfig.runtime_fallback;
    const runtimeFallback = isHookEnabled("runtime-fallback")
        ? safeHook("runtime-fallback", () => createRuntimeFallbackHook(ctx, {
