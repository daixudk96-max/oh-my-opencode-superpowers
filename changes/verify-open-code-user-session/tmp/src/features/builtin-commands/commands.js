...
import { CANCEL_RALPH_TEMPLATE, RALPH_LOOP_TEMPLATE, } from "./templates/ralph-loop";
...
function createRuntimeTemplate(originalTemplate, presetManager, agentChainManager) {
    return (args) => {
...
`;
        ...
        if (chainType && agentChainManager.hasChain(chainType)) {
            const description = userMessage
                .replace(/--chain(?:=|\s+)(bugfix|refactor)\b/gi, "")
                .replace(/\s+/g, " ")
                .trim();
            const chainContext = agentChainManager.generateExecutionContext(chainType, {
                description,
            });
...
export function loadBuiltinCommands(disabledCommands, options) {
    ...
    for (const [name, definition] of Object.entries(definitions)) {
        if (!disabled.has(name)) {
            ...
            const template = runtimeTemplates
                ? createRuntimeTemplate(originalTemplate, presetManager, agentChainManager)
                : originalTemplate;
            commands[name] = {
                ...openCodeCompatible,
                name,
