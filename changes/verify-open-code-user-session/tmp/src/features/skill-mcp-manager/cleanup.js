...
export function registerProcessCleanup(state) {
...
export function unregisterProcessCleanup(state) {
    if (!state.cleanupRegistered)
        return;
...
export function startCleanupTimer(state) {
...
export function stopCleanupTimer(state) {
    if (!state.cleanupInterval)
        return;
    clearInterval(state.cleanupInterval);
    state.cleanupInterval = null;
}
async function cleanupIdleClients(state) {
    const now = Date.now();
...
export async function disconnectSession(state, sessionID) {
    const keysToRemove = [];
...
export async function disconnectAll(state) {
    stopCleanupTimer(state);
...
export async function forceReconnect(state, clientKey) {
