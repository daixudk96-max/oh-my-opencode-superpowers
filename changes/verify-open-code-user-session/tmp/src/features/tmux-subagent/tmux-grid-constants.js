...
export function getMainPaneSizePercent(config) {
    return clamp(config?.mainPaneSize ?? DEFAULT_MAIN_PANE_SIZE, 20, 80);
}
export function computeMainPaneWidth(windowWidth, config) {
    ...
    const percentageMainPaneWidth = Math.floor((safeWindowWidth - dividerWidth) * (getMainPaneSizePercent(config) / 100));
    const maxMainPaneWidth = Math.max(0, safeWindowWidth - dividerWidth - minAgentPaneWidth);
    return clamp(Math.max(percentageMainPaneWidth, minMainPaneWidth), 0, maxMainPaneWidth);
}
export function computeAgentAreaWidth(windowWidth, config) {
    const safeWindowWidth = Math.max(0, windowWidth);
    if (!config) {
