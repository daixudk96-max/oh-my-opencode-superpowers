...
import { isInsideTmux as defaultIsInsideTmux, getCurrentPaneId as defaultGetCurrentPaneId, POLL_INTERVAL_BACKGROUND_MS, SESSION_READY_POLL_INTERVAL_MS, SESSION_READY_TIMEOUT_MS, } from "../../shared/tmux";
import { queryWindowState } from "./pane-state-querier";
import { decideSpawnActions, decideCloseAction } from "./decision-engine";
import { executeActions, executeAction } from "./action-executor";
import { TmuxPollingManager } from "./polling-manager";
const defaultTmuxDeps = {
...
export class TmuxSessionManager {
    ...
    async closeSessionById(sessionId) {
        ...
        if (state) {
            await executeAction({ type: "close", paneId: tracked.paneId, sessionId }, {
                config: this.tmuxConfig,
                serverUrl: this.serverUrl,
                windowState: state,
                sourcePaneId: this.sourcePaneId,
    ...
    async cleanup() {
        ...
        if (this.sessions.size > 0) {
            ...
            const state = this.sourcePaneId ? await queryWindowState(this.sourcePaneId) : null;
            if (state) {
                const closePromises = Array.from(this.sessions.values()).map((s) => executeAction({ type: "close", paneId: s.paneId, sessionId: s.sessionId }, {
                    config: this.tmuxConfig,
                    serverUrl: this.serverUrl,
                    windowState: state,
                    sourcePaneId: this.sourcePaneId,
                }).catch((err) => log("[tmux-session-manager] cleanup error for pane", {
                    paneId: s.paneId,
                    error: String(err),
                })));
