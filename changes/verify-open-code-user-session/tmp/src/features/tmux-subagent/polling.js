import { POLL_INTERVAL_BACKGROUND_MS, SESSION_MISSING_GRACE_MS, } from "../../shared/tmux";
import { log } from "../../shared";
import { queryWindowState } from "./pane-state-querier";
import { executeAction } from "./action-executor";
import { MIN_STABILITY_TIME_MS, SESSION_TIMEOUT_MS, STABLE_POLLS_REQUIRED, } from "./polling-constants";
...
export function createSessionPollingController(params) {
    ...
    async function closeSessionById(sessionId) {
        ...
        if (state) {
            await executeAction({ type: "close", paneId: tracked.paneId, sessionId }, { config: params.tmuxConfig, serverUrl: params.serverUrl, windowState: state });
        }
                        ...
                        if (tracked.lastMessageCount === currentMessageCount) {
                            ...
                            if (tracked.stableIdlePolls >= STABLE_POLLS_REQUIRED) {
                                ...
                                if (recheckStatus?.type === "idle") {
                                    ...
                                    log("[tmux-session-manager] stability reached but session not idle on recheck, resetting", { sessionId, recheckStatus: recheckStatus?.type });
                                }
                            }
                        }
                        else {
