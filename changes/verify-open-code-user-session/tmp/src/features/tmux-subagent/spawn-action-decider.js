...
import { canSplitPane, findMinimalEvictions, isSplittableAtCount, } from "./pane-split-availability";
import { findSpawnTarget } from "./spawn-target-finder";
import { findOldestAgentPane } from "./oldest-agent-pane";
function getInitialSplitDirection(layout) {
    return layout === "main-horizontal" ? "-v" : "-h";
}
function isStrictMainLayout(layout) {
...
export function decideSpawnActions(state, sessionId, description, config, sessionMappings) {
    ...
    const canEvaluateSpawnTarget = strictLayout ||
...
export function decideCloseAction(state, sessionId, sessionMappings) {
