...
export async function handleSessionCreated(deps, event) {
    ...
    try {
        ...
        const decision = decideSpawnActions(state, sessionId, title, deps.getCapacityConfig(), deps.getSessionMappings());
        ...
        if (!sessionReady) {
            ...
            await executeActions([{ type: "close", paneId: result.spawnedPaneId, sessionId }], {
                config: deps.tmuxConfig,
                serverUrl: deps.serverUrl,
                windowState: state,
            });
