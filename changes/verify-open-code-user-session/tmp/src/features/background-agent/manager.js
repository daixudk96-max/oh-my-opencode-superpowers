...
import { log, getAgentToolRestrictions, normalizePromptTools, normalizeSDKResponse, promptWithModelSuggestionRetry, resolveInheritedPromptTools, createInternalAgentTextPart, } from "../../shared";
...
import { isAbortedSessionError, extractErrorName, extractErrorMessage, getSessionErrorMessage, isRecord, } from "./error-classifier";
...
export class BackgroundManager {
    ...
    pendingByParent; // Track pending tasks per parent for batching
    client;
    directory;
    pollingInterval;
    ...
    handleEvent(event) {
        ...
        if (event.type === "session.error") {
            ...
            const canRetry = shouldRetryError(errorInfo) &&
                !!task.fallbackChain &&
        ...
        try {
            ...
            if (task) {
                ...
                if (lastAssistantMessage && Array.isArray(lastAssistantMessage.parts)) {
                    ...
                    if (fullText) {
                        task.result = formatHandover(parseHandover(fullText));
                    }
                }
            }
            return true;
    ...
    async tryCompleteTask(task, source) {
        ...
        // task.result is populated during output validation paths (session.idle / polling)
        // to avoid duplicate session.messages() calls during completion.
        this.markForNotification(task);
            ...
            try {
                ...
                for (let i = messages.length - 1; i >= 0; i--) {
                    ...
                    const normalizedTools = isRecord(info?.tools)
                        ? normalizePromptTools(info.tools)
                        : undefined;
