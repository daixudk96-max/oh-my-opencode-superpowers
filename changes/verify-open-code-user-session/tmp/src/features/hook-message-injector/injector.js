 ...
 */
export async function findNearestMessageWithFieldsFromSDK(client, sessionID) {
    try {
 ...
 */
export async function findFirstMessageWithAgentFromSDK(client, sessionID) {
    try {
        const response = await client.session.messages({ path: { id: sessionID } });
        const messages = normalizeSDKResponse(response, [], { preferResponseOnMissingData: true });
 ...
 */
export function findNearestMessageWithFields(messageDir) {
 ...
 */
export function findFirstMessageWithAgent(messageDir) {
    // On beta SQLite backend, skip JSON file reads entirely
    if (isSqliteBackend()) {
        return null;
    }
 ...
 */
export function injectHookMessage(sessionID, hookContent, originalMessage) {
    ...
    const needsFallback = !originalMessage.agent ||
...
export async function resolveMessageContext(sessionID, client, messageDir) {
