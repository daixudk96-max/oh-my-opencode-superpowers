...
export async function getMainSessions(options) {
    // Beta mode: use SDK
...
export async function readSessionTodos(sessionID) {
    // Beta mode: use SDK
    if (isSqliteBackend() && sdkClient) {
        try {
...
export async function readSessionTranscript(sessionID) {
...
export async function getSessionInfo(sessionID) {
    ...
    for (const msg of messages) {
        ...
        if (msg.time?.created) {
            ...
            if (!firstMessage || date < firstMessage)
                firstMessage = date;
            if (!lastMessage || date > lastMessage)
                lastMessage = date;
        }
    }
