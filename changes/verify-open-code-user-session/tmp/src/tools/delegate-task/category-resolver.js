...
export async function resolveCategoryExecution(args, executorCtx, inheritedModel, systemDefaultModel) {
...
Available categories: ${allCategoryNames}`,
    ...
    if (!requirement) {
        ...
        if (resolution) {
            ...
            const type = (explicitCategoryModel || overrideModel)
                ? "user-defined"
                : (systemDefaultModel && actualModel === systemDefaultModel)
                    ? "system-default"
                    : "category-default";
            const source = type === "user-defined"
                ? "override"
                : type === "system-default"
                    ? "system-default"
                    : "category-default";
