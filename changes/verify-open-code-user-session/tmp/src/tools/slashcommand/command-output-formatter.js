...
export async function formatLoadedCommand(command, userMessage) {
    const sections = [];
    ...
    let content = typeof command.content === "function"
        ? command.content({ user_message: userMessage })
        : (command.content ?? "");
...
export function formatCommandList(items) {
    if (items.length === 0)
    ...
    for (const command of items) {
        ...
        lines.push(`- **/${command.name}${hint}**: ${command.metadata.description || "(no description)"} (${command.scope})`);
    }
