...
function normalizeAdditionalHookNames(additionalHookNames) {
    const names = new Set();
...
export const createHookNameSchema = (additionalHookNames = []) => {
    ...
    const additionalHookLiterals = normalizedHookNames.map((hookName) => z.literal(hookName));
    return z.union([HookNameSchema, ...additionalHookLiterals]);
};
export function parseHookName(value, additionalHookNames = []) {
