...
export class BackgroundManager {
  ...
  private async validateSessionHasOutput(sessionID: string): Promise<boolean> {
    try {
      ...
      if (task) {
        ...
        if (lastAssistantMessage && Array.isArray((lastAssistantMessage as { parts?: unknown }).parts)) {
          const textParts = (lastAssistantMessage as { parts: Array<{ type?: unknown; text?: unknown }> }).parts
  ...
  private async notifyParentSession(task: BackgroundTask): Promise<void> {
    ...
    const completedTasks: BackgroundTask[] = allComplete
