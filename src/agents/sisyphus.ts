...
import { isGeminiModel, isGpt5_4Model, isGptModel } from "./types";
...
Pass \`session_id="{session_id}"\` to continue a previous agent with FULL CONTEXT PRESERVED.

**ALWAYS use session_id when:**
- Previous task failed → \`session_id="{session_id}", prompt="Fix: [specific error]"\`
- Need follow-up on result → \`session_id="{session_id}", prompt="Also check [additional query]"\`
- Multi-turn with same agent → reuse \`session_id\` instead of starting fresh (saves tokens!)

**Example:**
\`\`\`
task(session_id="ses_abc123", load_skills=[], run_in_background=false, description="Follow-up search", prompt="The previous search missed X. Also look for Y.")
\`\`\`
