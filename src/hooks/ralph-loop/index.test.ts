...
describe("ralph-loop", () => {
			...
			client: {
				session: {
					...
					messages: async (opts: { path: { id: string } }) => {
						...
						return mockMessagesApiResponseShape === "array"
							? mockSessionMessages
	...
	describe("command arguments", () => {
		test("should parse --strategy=reset flag", () => {
			...
			const rawArguments =
				'"Build feature X" --strategy=reset --max-iterations=12';
	...
	describe("hook", () => {
		...
		test("should detect completion promise and stop loop", async () => {
			...
			writeFileSync(
				transcriptPath,
				`${JSON.stringify({ type: "tool_result", tool_name: "write", tool_output: { output: "Task done\n<promise>COMPLETE</promise>" } })}\n`,
			);
			...
			expect(promptCalls.length).toBe(0);
			expect(toastCalls.some((t) => t.title === "Ralph Loop Complete!")).toBe(
				true,
			);
			expect(hook.getState()).toBeNull();
		});
			...
			mockSessionMessages = [
				...
				},
				{
					...
					parts: [
						{
							type: "text",
							text: "I have completed the task. <promise>API_DONE</promise>",
						},
					],
				},
			];
		...
		test("should detect completion promise from assistant tool_result part via session messages API", async () => {
			// given - active loop with assistant tool_result containing completion promise
			mockSessionMessages = [
				{
					info: { role: "user" },
					parts: [{ type: "text", text: "Build something" }],
				},
				{
					...
					parts: [
						{
							type: "tool_result",
							output: "Task finished. <promise>API_DONE</promise>",
						},
					],
				},
			];
			...
			// then - loop completed via assistant tool_result detection
			expect(promptCalls.length).toBe(0);
			expect(toastCalls.some((t) => t.title === "Ralph Loop Complete!")).toBe(
				true,
			);
			...
			mockSessionMessages = [
				...
				},
				{
					...
					parts: [
						{
							type: "text",
							text: "When fully complete, output <promise>DONE</promise>",
						},
					],
				},
			];
		...
		test("should detect completion promise in reasoning part via session messages API", async () => {
			//#given - current detector treats reasoning text as assistant output
			mockSessionMessages = [
				{
					info: { role: "user" },
					parts: [{ type: "text", text: "Build something" }],
				},
				{
					...
					parts: [
						{
							type: "reasoning",
							text: "I am done now. <promise>REASONING_DONE</promise>",
						},
					],
				},
			];
			...
			//#then - loop completes because reasoning text is currently scanned
			expect(promptCalls.length).toBe(0);
			expect(toastCalls.some((t) => t.title === "Ralph Loop Complete!")).toBe(
				true,
			);
		...
		test("should include prompt and promise in continuation message", async () => {
			...
			expect(promptCalls[0].text).toContain(
				"<promise>CALCULATOR_DONE</promise>",
			);
		});
		...
		test("should skip concurrent idle events for same session when handler is in flight", async () => {
			...
			const hook = createRalphLoopHook(
				mockInput as Parameters<typeof createRalphLoopHook>[0],
			);
		...
		test("should check last 3 assistant messages for completion", async () => {
			...
			mockSessionMessages = [
				...
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "(extra output after promise)" }],
				},
			];
			const hook = createRalphLoopHook(createMockPluginInput(), {
				getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
		...
		test("should detect completion even when promise is older than previous narrow window", async () => {
			...
			mockSessionMessages = [
				...
				},
				{
					info: { role: "assistant" },
					parts: [
						{ type: "text", text: "Promise early <promise>DONE</promise>" },
					],
				},
				{ info: { role: "user" }, parts: [{ type: "text", text: "Continue" }] },
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "More work 1" }],
				},
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "More work 2" }],
				},
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "More work 3" }],
				},
			];
			const hook = createRalphLoopHook(createMockPluginInput(), {
				getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
		...
		test("should detect completion when many assistant messages are emitted after promise", async () => {
			...
			mockSessionMessages = [
				...
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "Done now <promise>DONE</promise>" }],
				},
			];
			...
			for (let index = 1; index <= 25; index += 1) {
				mockSessionMessages.push({
					info: { role: "assistant" },
					parts: [
						{ type: "text", text: `Post-completion assistant output ${index}` },
					],
				});
			}
...
Original task: Build something`;
		...
		test("should ignore transcript completion entries from before loop start", async () => {
			// given - transcript contains an old completion promise before the current loop started
			const transcriptPath = join(TEST_DIR, "transcript.jsonl");
			const oldAssistantEntry = JSON.stringify({
				type: "assistant",
				timestamp: "2000-01-01T00:00:00.000Z",
				content: "Old completion <promise>DONE</promise>",
			});
			writeFileSync(transcriptPath, `${oldAssistantEntry}\n`);
			...
			// then - loop should continue because old transcript entries are ignored
			expect(promptCalls.length).toBe(1);
			expect(toastCalls.some((t) => t.title === "Ralph Loop Complete!")).toBe(
				false,
			);
		...
		test("should check transcript BEFORE API to optimize performance", async () => {
			...
			writeFileSync(
				transcriptPath,
				`${JSON.stringify({ type: "tool_result", tool_name: "write", tool_output: { output: "<promise>DONE</promise>" } })}\n`,
			);
			mockSessionMessages = [
				{
					info: { role: "assistant" },
					parts: [{ type: "text", text: "No promise here" }],
				},
			];
			const hook = createRalphLoopHook(createMockPluginInput(), {
				getTranscriptPath: () => transcriptPath,
		...
		test("should require oracle verification toast for ultrawork completion promise", async () => {
			...
			await hook.event({
				event: { type: "session.idle", properties: { sessionID: "test-id" } },
			});

			// then - loop transitions to oracle verification instead of immediate completion
			const verificationToast = toastCalls.find(
				(t) => t.title === "ULTRAWORK LOOP",
			);
			expect(verificationToast).toBeDefined();
			expect(verificationToast?.message).toMatch(
				/Oracle verification is now required/,
			);
		});
	...
	describe("API timeout protection", () => {
		test("should not hang when session.messages() throws", async () => {
			...
			const hook = createRalphLoopHook(
				errorMock as unknown as Parameters<typeof createRalphLoopHook>[0],
				{
					getTranscriptPath: () => join(TEST_DIR, "nonexistent.jsonl"),
					apiTimeout: 100,
				},
