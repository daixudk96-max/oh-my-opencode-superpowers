...
`,
		...
		expect(
			readBoulderState(testDirectory)?.session_ids.includes(SESSION_ID),
		).toBe(true);
	});

	it("skips continuation when only nested checklist rows remain unchecked", async () => {
		writeBoulderStateForPlan(
			"complete-top-level-nested-checklists",
			`## Phase 1: Setup
- [x] Ship parser fix
